...
main = do
         let johnTheWorkHorse = Worker "333-12-1200" "John"
                                   Nothing "Smith" ["1234 Amber Lane",
                                   "St. Louis", "MO"]

         let carlTheSlouch = Worker{ssn = "001-30-3032",
                                addresses = ["803 Amber Lane", "Apt B",
                                            "Little Rock", "AR"],
                                firstName = "Carl",
         ...
         putStrLn $ show $ assert
            (firstName johnTheWorkHorse == "John") "Success"
         putStrLn $ show $ assert (firstName bobTheBoss == "Bob") "Success"
         putStrLn $ show $ assert (firstName carlTheSlouch == "Carl") "Success"