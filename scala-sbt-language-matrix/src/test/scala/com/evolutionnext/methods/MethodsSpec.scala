...
class MethodsSpec extends AnyFunSuite with Matchers {
  ...
  test("""Recursion is supported just like another language, here
  ...
  test("""The above was ugly, so let's clean it up with
  ...
  test("""Repeated parameters are the equivalent of varargs in Java, they
  ...
  test("""Repeated parameters can be sent a list or any other collection,
  ...
  test("""Default methods have just methods that have a value
