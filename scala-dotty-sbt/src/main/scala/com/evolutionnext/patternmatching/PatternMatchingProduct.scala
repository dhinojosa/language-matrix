...
@main def patternMatchingWords: Unit =
  val books = Seq("Programming Scala", "JavaScript: The Good Parts", "Scala Cookbook").zipWithIndex // add an "index"

  val result = books.map { case Words(words, index) =>
    s"$index: count = ${words.size}"
  }
